...
ssr_value = lib.permanent_character_tw_Quantity_ssr

r = 79.5*0.01 #
sr = 18*0.01 #
ssr = 2.5*0.01 #
card_num = lib.permanent_character_tw_Quantity * (r_value * sr_value * ssr_value)  * 1000  ##所有常駐角色
...
def rare_by_feat_r(num,feat_value):
    ...
    for i in range(feat_value+1):
        if(num<=status * (i+1) + 1):
            #result.append(weight)
            result.append(lib.draw_feat_data_tw_nor_r[i])
            break
    return result
#############################################################
##分配抽到的角色(金卡)
